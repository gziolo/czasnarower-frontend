  ...
  describe('User race stats', function() {
    ...
    describe('Data provided', function() {
      ...
      it('has datagrid with two rows', function() {
        this.instance.$node.find('table.datagrid tbody tr').length.should.equal(2);
      });

      it('has datagrid with five columns', function() {
        this.instance.$node.find('table.datagrid tbody tr:first td').length.should.equal(5);
      });

      it('has datagrid sorted by total time column', function() {
        this.instance.$node.find('table.datagrid tbody tr:first > td:nth-child(2)').text().should.equal('05:02:33');
      });

      it('has datagrid with image in first column', function() {
        this.instance.$node.find('table.datagrid tbody tr:first > td:nth-child(1) img').attr('src').should.equal('http://www.czasnarower.pl/photo/avatar/avatar_default.jpg');
      });

      it('has datagrid with link to user profile in first column', function() {
        this.instance.$node.find('table.datagrid tbody tr:first > td:nth-child(1) a').attr('href').should.equal('http://www.czasnarower/uzytkownik/323');
      });
