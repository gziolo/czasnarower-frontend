    ...
    .register(
        ...
        function(facade, $) {
            ...
            facade.ajax({
              ...
              success : function(oData) {
                if (!Number(oData.add.iStatus)) {
                  ...
                  // oForm.append('<div
                  // class="communique"><p>'+oData.add.sMessage+'</p></div>');
                  // oForm.find('.communique').show();
                  // oForm.find('.communique').delay(2000).fadeOut( 100,
          ...
          function _bindConfirmRemove(params) {
            ...
            var html = "<p>Czy na pewno chcesz usunąć wiadomość?</p><button class='btn btn-small btn-danger confirm-remove'>Usuń</button><button class='btn btn-small cancel-remove'>Anuluj</button>";
            elem.popover({
              html : true,
              content : html,
              title : 'Potwierdzenie',
          ...
          function bindButtons() {
            ...
            /*
             * $('body').on('click', '.message .remove-message', function() {
             * var elem = $(this); var params = { id : elem.attr('data-id') };
