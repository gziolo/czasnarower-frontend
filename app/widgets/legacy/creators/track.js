/*global Core, google */
Core.Creator.register('track', function(facade, $) {
  ...
  /**
   * array with main markers which connect direction-tracks and simple polylines
   */
  aMarkers = [],
  ...
  /**
   * object LatLngBounds polyBounds for keeping information about current polys
   * bounds
   */
  polysBounds,
  /**
  ...
  function bindForm() {
    ...
    // $(this).closest('.control-group').removeClass('alert error alert-error');
    // $('#title_communique').hide();
    });
  }

  /**
   * every new added point must be handled put marker on the map create new poly
   * and add to the map
   */
  ...
  /**
   * method for creating new google-maps-marker object and - binds proper events -
   * sets unique id
   */
  ...
  function dividePoly(divisor) {
    ...
    moveMarkers(index + 1, 1, [ marker ]); // insert marker at position index+1
