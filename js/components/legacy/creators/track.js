    ...
    return function(sandbox, $) {
        ...
        function formatGeocoderResult(result) {
            ...
            var allowed_types = [
                ...
                'natural_feature', 'park', 'point_of_interest', 'landmark'
            ];
            for (var index = 0; index < addr_components.length; index++) {
                var component = addr_components[index];
                if ($.inArray('administrative_area_level_1', component.types) > -1) { // województwo
                    ret.administrativeRegion = component;
                    ret.name += ' (' + component.short_name + ')';
                }
                if ($.inArray('country', component.types) > -1) { // kraj
                    ret.country = component;
                }
                if ($.inArray('administrative_area_level_2', component.types) > -1) { // powiat
                ...
                if ($.inArray('locality', component.types) > -1) { // miasto
                    ret.locality = component;
                }
                if (component.long_name) {
                    var types = component.types;
                    if ($(types).filter(allowed_types).length > 0) {
        ...
        function getCoordinates() {
            ...
            var path = [];
            var isGpxOriginal = (fileSrc !== '' && !modified) ? true : false;
            var params = {
                geoTags: [],
                startPlace: '',
                country: '',
            ...
            var isAltitude = isGpxOriginal; // Try to get data only if user marked the trail

            var saveIfReady = function() {
                if (numGeocoded === limitGeocoded && startGeocoded && isAltitude) {
                    saveCoordinates(button, params);
                }
            }

            // Geocode start point and set start place
            geocoder.geocode({
                'location': path[0]
            }, function(results, status) {
                if (status === google.maps.GeocoderStatus.OK) {
            ...
            while (count--) {
                ...
                geocoder.geocode({ 'latLng': latLng }, geocoderResult);
            }
            ...
            if (!isAltitude) {
                ...
                var i, j, chunk = 512;
                var paths = [];
                if (path.length > chunk) {
                    for (i = 0, j = path.length; i < j; i += chunk) {
                        paths.push(path.slice(i, i + chunk));
                    }
                } else {
                    paths = [path];
                }
                ...
                var getElevations = function() {
                    elevationService.getElevationForLocations({ locations: paths[currentIndex] }, elevationResult);
                }
                var elevationResult = function(results, status) {
                    if (status === google.maps.ElevationStatus.OK) {
                        $.each(results, function(i, alt) {
                            params.altitudes.push(alt.elevation);
                        });
                    } else {
                        params.altitudes = []; // reset and quit
                        isAltitude = true;
                    }
                    currentIndex += 1;
                    if (numReq > currentIndex && !isAltitude) {
                        getElevations();
                    } else {
                        isAltitude = true;
                    }
